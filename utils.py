...
import nltk
import unidecode
from pygoogletranslation.constants import (LANGCODES, LANGUAGES, RPCIDS)
...
def format_response_legacy(a):
  ...
  for _b in b:
    ...
    if flag:
      li_filter.append(_b)
  fi_data = str(''.join(li_filter)).replace('"[',
                                            '[').replace(']"', ']').replace(
                                              '\\n', '').replace('\\', '')
  li_data = json.loads(
    fi_data.split('pygoogletranslation')[1].replace('"[',
                                                    '[').replace(']"', ']'))
  return li_data
...
def format_response(datalist):
  ...
  for data in datalist:
    ...
    translated_parts = list(
      map(
        lambda part: TranslatedPart(part[0], part[1]
                                    if len(part) >= 2 else []),
        parsed[1][0][0][5]))
